...
const App = () => {
  ...
  const handleFormSubmit = (e) => {
    e.preventDefault();
    const totalRooms = numPremiumRooms + numEconomyRooms;
    const totalGuests = guestData.length;

    if (totalRooms < totalGuests) {
      alert("Error: Not enough rooms available for all guests.");
      return;
    }
    const [premiumGuests, economyGuests] = assignRooms(
      guestData,
      numPremiumRooms,
      numEconomyRooms
    );
      ...
      <div className="admin-dash">
        <form onSubmit={handleFormSubmit} className="form-submit">
          <div>
            <FormField
              label="Premium Rooms"
              value={numPremiumRooms}
              onChange={(e) => setNumPremiumRooms(parseInt(e.target.value))}
            />
          </div>
          <div>
            <FormField
              label="Economy Rooms"
              value={numEconomyRooms}
              onChange={(e) => setNumEconomyRooms(parseInt(e.target.value))}
            />
          </div>
          <div>
            <button type="submit" className="optmize">
              Optimize
            </button>
          </div>
        </form>
